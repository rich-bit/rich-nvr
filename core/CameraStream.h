#pragma once
#include "SegmentWorker.h" // For buffer segments
#include "Settings.h"
#include <chrono>
#include <gst/gst.h>          // For GstElement, GstSample, etc.
#include <opencv2/opencv.hpp> // For cv::Mat, cv::imencode, etc.
#include <string>
...
struct AudioProbeResult {
  ...
  std::string encoding; // e.g. "MPEG4-GENERIC", "PCMU", "OPUS"
  int channels = 0;
  int rate = 0;
  bool probed = false;
};
...
public:
  CameraStream(const std::string &name, const std::string &uri,
               ...
               float motion_frame_scale = 1.0f, // New
  ...
  void start(); // Now doesn't take GMainLoop*
  void stop();
...
private:
  ...
  float motion_frame_scale_ = 1.0f; // New: for scale, 1.0 = 100%
  bool motionDetected_ = false;
  bool prevMotionDetected_ = false;
  std::string video_output_format_ = "mp4";
  std::string output_path_;

  cv::Mat last_motion_frame_; // For raw frame storage (if you want to
  // access/process)
  cv::Size motion_frame_size_{0, 0}; // 0,0 means "no resize" (use native)
  std::vector<uchar> last_jpeg_buf_; // For encoded JPEG bytes
