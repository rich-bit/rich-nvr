...
ConfigurationPanel::ConfigurationPanel(
    ...
    PersistCallback persist_callback, AddCameraCallback add_camera_callback,
    ...
    std::function<bool(const std::string &, void *&, int &, int &)>
        fetch_motion_frame_callback,
    std::function<int(const std::string &, int, int, int, int, float)>
        add_motion_region_callback,
    ...
    std::function<std::vector<MotionRegion>(const std::string &)>
        get_motion_regions_callback)
    : auto_reconnect_(true), show_fps_overlay_(false), master_volume_(0.75f),
      alert_volume_(0.9f), motion_detection_enabled_(true),
      motion_threshold_(0.45f), motion_window_size_(12),
      active_tab_(Tab::General), requested_tab_(Tab::General),
      needs_tab_selection_(false), window_settings_(window_settings),
      ...
      window_size_dirty_(false), add_camera_via_server_(true),
      add_camera_segment_(false), add_camera_overlay_(false),
      add_camera_motion_frame_(true), add_camera_live555_proxy_(true),
      add_camera_motion_frame_scale_(1.0f), add_camera_noise_threshold_(0.3f),
      add_camera_motion_threshold_(0.5f), add_camera_motion_min_hits_(4),
      add_camera_motion_decay_(20), add_camera_motion_arrow_scale_(1.0f),
      add_camera_motion_arrow_thickness_(2), add_camera_limit_frame_rate_(true),
      add_camera_status_success_(false), server_health_checking_(false),
      server_health_available_(false), server_health_camera_count_(0),
      server_health_uptime_(0), server_health_rtsp_port_(0),
      last_health_check_time_(0.0f), proxy_initiate_in_progress_(false),
      proxy_initiated_successfully_(false), proxy_probe_in_progress_(false),
      last_proxy_probe_time_(0.0f), selected_camera_index_(0),
      motion_frame_enabled_(false), motion_frame_texture_(nullptr),
      motion_frame_width_(0), motion_frame_height_(0),
      last_motion_frame_fetch_(0.0f), drawing_motion_region_(false),
      region_draw_start_x_(0), region_draw_start_y_(0), region_draw_end_x_(0),
      region_draw_end_y_(0), pending_region_angle_(0.0f),
      selected_region_index_(0), last_region_fetch_time_(0.0f),
      show_record_motion_warning_(false),
      dont_show_record_motion_warning_(false), async_worker_(nullptr),
      probe_in_progress_(false), close_after_save_(false),
      last_server_camera_fetch_time_(0.0f),
      server_camera_fetch_in_progress_(false),
      motion_frame_fetch_in_progress_(false), has_pending_motion_frame_(false),
      pending_motion_frame_width_(0), pending_motion_frame_height_(0),
      motion_frame_fetch_interval_(1.0f),
      server_thread_info_fetch_in_progress_(false),
      last_server_thread_info_fetch_(0.0f), server_check_interval_(5.0f),
      rtsp_config_stream_index_(-1), show_rtsp_config_popup_(false),
  ...
  std::snprintf(add_camera_name_.data(), add_camera_name_.size(), "%s",
                "Camera");
  std::snprintf(add_camera_rtsp_.data(), add_camera_rtsp_.size(), "%s",
                "rtsp://");

  const char *endpoint = default_server_endpoint.empty()
                             ? "http://localhost:8080"
                             : default_server_endpoint.c_str();
  std::snprintf(server_endpoint_.data(), server_endpoint_.size(), "%s",
                endpoint);
}
...
void ConfigurationPanel::render(bool &open) {
  ...
  window_settings_.width =
      std::clamp(window_settings_.width, min_window_width, max_window_width);
  window_settings_.height =
      std::clamp(window_settings_.height, min_window_height, max_window_height);

  // Set initial size only once, then allow manual resizing
  ImGui::SetNextWindowSize(
      ImVec2(window_settings_.width, window_settings_.height), ImGuiCond_Once);
  ImGui::SetNextWindowSizeConstraints(
      ImVec2(min_window_width, min_window_height),
      ImVec2(max_window_width, max_window_height));
  ...
  if (ImGui::BeginTabBar("ConfigurationTabs")) {
    ...
    renderAddCameraTab(needs_tab_selection_ &&
                       requested_tab_ == Tab::AddCamera);
    renderMotionFrameTab(needs_tab_selection_ &&
                         requested_tab_ == Tab::MotionFrame);
  ...
  if (ImGui::BeginPopupModal("Record on Motion Warning", nullptr,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::TextWrapped("This will constantly record video segments and wait "
                       "for motion events.");
    ImGui::Spacing();
    ImGui::TextWrapped("Warning: This involves constant disk writes which can "
                       "cause significant");
    ImGui::TextWrapped("wear on your storage drive. Use only if you understand "
                       "that your drive");
    ...
    ImGui::Checkbox("Don't show this warning again",
                    &dont_show_record_motion_warning_);
...
void ConfigurationPanel::setRTSPConfigCallbacks(
    GetRTSPConfigCallback get_callback, SaveRTSPConfigCallback save_callback,
...
void ConfigurationPanel::renderRTSPConfigPopup() {
  ...
  if (ImGui::BeginPopupModal("RTSP Stream Configuration",
                             &show_rtsp_config_popup_,
                             ImGuiWindowFlags_NoSavedSettings)) {
    // Determine if we're in Add Camera mode (stream_index == -1) or editing
    // existing stream
    bool is_add_camera_mode = (rtsp_config_stream_index_ == -1);

    // Get reference to config (use temp for Add Camera mode, or callback for
    // existing stream)
    ...
    if (is_add_camera_mode) {
      ...
      ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f),
                         "Error: RTSP config callbacks not set");
    ...
    if (ImGui::Combo("Transport Protocol", &transport_idx, transport_options,
                     2)) {
      config.rtsp_transport = (transport_idx == 1) ? "udp" : "tcp";
    }
    ImGui::TextWrapped("TCP: More reliable, higher latency. UDP: Lower "
                       "latency, may drop packets.");
    ImGui::Spacing();

    ImGui::SliderInt("Timeout (seconds)", &config.rtsp_timeout_seconds, 1, 30);
    ImGui::TextWrapped(
        "How long to wait for connection/read operations before giving up.");
    ImGui::Spacing();

    ImGui::SliderInt("Max Delay (ms)", &config.max_delay_ms, 100, 5000);
    ImGui::TextWrapped("Maximum demuxing delay. Lower = less latency, higher = "
                       "more buffering.");
    ImGui::Spacing();

    ImGui::SliderInt("Buffer Size (KB)", &config.buffer_size_kb, 128, 8192);
    ImGui::TextWrapped(
        "Network receive buffer size. Increase for unstable connections.");
    ImGui::Spacing();
    ...
    ImGui::TextWrapped(
        "Skip B-frames and reduce buffering for lowest possible latency.");
    ImGui::Spacing();

    ImGui::Checkbox("Disable Internal Buffering", &config.fflags_nobuffer);
    ImGui::TextWrapped("Disable FFmpeg's internal buffering. Usually faster "
                       "but may be unstable.");
    ImGui::Spacing();

    ImGui::SliderInt("Probe Size (KB)", &config.probesize_kb, 100, 10000);
    ImGui::TextWrapped("How much data to analyze when opening stream. Lower = "
                       "faster connect.");
    ImGui::Spacing();

    ImGui::SliderInt("Analyze Duration (ms)", &config.analyzeduration_ms, 100,
                     10000);
    ImGui::TextWrapped(
        "How long to analyze stream when opening. Lower = faster connect.");
    ImGui::Spacing();

    ImGui::SeparatorText("Hardware Acceleration");

    const char *hwaccel_options[] = {"None (Software)", "Auto", "CUDA (NVIDIA)",
    ...
    ImGui::TextWrapped(
        "Use GPU for video decoding. May not work on all systems.");
    ImGui::Spacing();
    ...
    const char *save_button_text =
        is_add_camera_mode ? "Save Settings" : "Save & Reload Stream";

    if (ImGui::Button(save_button_text, ImVec2(200, 0))) {
      if (is_add_camera_mode) {
        // In Add Camera mode, just close the popup (settings are saved in
        // rtsp_config_temp_)
        show_rtsp_config_popup_ = false;
      } else {
...
void ConfigurationPanel::renderGeneralTab(bool set_selected) {
  ...
  if (ImGui::BeginTabItem("General", nullptr, flags)) {
    ...
    bool metrics_changed = ImGui::Checkbox(
        "Show ImGui Metrics", &window_settings_.show_imgui_metrics);
    if (metrics_changed) {
    ...
    ImGui::Text("Window Size: %.0fx%.0f", window_settings_.width,
                window_settings_.height);
...
void ConfigurationPanel::renderAddCameraTab(bool set_selected) {
  ...
  if (ImGui::BeginTabItem("Add Camera", nullptr, flags)) {
    ...
    if (ImGui::Checkbox("Connect through NVR Server",
                        &add_camera_via_server_)) {
    ...
    if (ImGui::InputText("RTSP address", add_camera_rtsp_.data(),
                         add_camera_rtsp_.size())) {
    ...
    if (!add_camera_via_server_) {
      ...
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Configure RTSP transport, timeouts, buffering, and "
                          "hardware acceleration");
      }
    }

    // Show frame rate limiting checkbox for ALL cameras (both direct and
    // via-server)
    ImGui::Checkbox("Limit frame rate to stream's native FPS",
                    &add_camera_limit_frame_rate_);
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Prevents video from playing too fast and improves A/V "
    ...
    if (probe_in_progress_) {
      ...
      ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f), "%dx%d%s",
                         last_probe_result_.width, last_probe_result_.height,
    ...
    ImGui::InputText("Camera name", add_camera_name_.data(),
                     add_camera_name_.size());

    ImGui::BeginDisabled(!add_camera_via_server_);
    ImGui::InputText("Server endpoint", server_endpoint_.data(),
                     server_endpoint_.size());

    // Server health check (async, every 3 seconds)
    if (add_camera_via_server_ && async_worker_) {
      float current_time = ImGui::GetTime();
      if (current_time - last_health_check_time_ > 3.0f &&
          !server_health_checking_) {
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("Continuously records video segments and saves only "
    ...
    ImGui::DragFloat("Motion frame scale", &add_camera_motion_frame_scale_,
                     0.01f, 0.1f, 2.0f, "%.2f");
    ImGui::DragFloat("Noise threshold", &add_camera_noise_threshold_, 0.01f,
                     0.0f, 5.0f, "%.2f");
    ImGui::DragFloat("Motion threshold", &add_camera_motion_threshold_, 0.01f,
                     0.0f, 5.0f, "%.2f");
    ...
    ImGui::DragFloat("Motion arrow scale", &add_camera_motion_arrow_scale_,
                     0.01f, 0.0f, 5.0f, "%.2f");
    if (ImGui::InputInt("Motion arrow thickness",
                        &add_camera_motion_arrow_thickness_)) {
    ...
    if (!add_camera_via_server_) {
      ImGui::TextDisabled(
          "Advanced options are available when routing through NVR Server.");
    }
    ...
    if (needs_proxy && proxy_initiated_successfully_ &&
        !last_probe_result_.success && !proxy_probe_in_progress_ &&
        async_worker_ && !proxied_rtsp_url_.empty()) {
      float current_time = ImGui::GetTime();
    ...
    if (needs_proxy) {
      ImGui::BeginDisabled(proxy_initiate_in_progress_ ||
                           !server_health_available_);
      if (ImGui::Button("Initiate Proxy")) {
        if (async_worker_) {
          ...
          async_worker_->enqueueTask([this, request]() {
            ...
            auto result =
                client_network::send_add_camera_request(request, response_body);
    ...
    bool save_disabled = needs_proxy && (!proxy_initiated_successfully_ ||
                                         !last_probe_result_.success);
    ...
    if (ImGui::Button("Save")) {
      if (!add_camera_callback_) {
        ...
        // Use proxied URL if proxy was initiated, otherwise use original RTSP
        // URL
        if (needs_proxy && proxy_initiated_successfully_ &&
            !proxied_rtsp_url_.empty()) {
          request.rtsp_url = proxied_rtsp_url_;
          // Keep connect_via_server = true for proxied cameras so they appear
          // in Motion Frames tab The client will connect directly to the
          // proxied URL, but the camera is still managed by the server
          request.connect_via_server = true;
        } else {
          request.rtsp_url = add_camera_rtsp_.data();
        ...
        request.live555_proxy =
            false; // Always false when saving (proxy already initiated)
    ...
    if (needs_proxy) {
      if (proxy_initiate_in_progress_) {
        ...
        ImVec4 color = proxy_initiated_successfully_
        ...
        if (proxy_initiated_successfully_) {
          if (proxy_probe_in_progress_) {
            ...
            ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                               "Probe: Failed (retrying every 2s)");
            if (ImGui::IsItemHovered()) {
              ImGui::SetTooltip("%s", last_probe_result_.error_message.c_str());
            }
          } else {
    ...
    if (!add_camera_status_.empty()) {
      ImGui::Spacing();
      ImVec4 color = add_camera_status_success_
...
void ConfigurationPanel::decode_motion_frame_from_buffer_(
    const std::vector<unsigned char> &jpeg_buffer, void *&texture_out,
...
void ConfigurationPanel::renderMotionFrameTab(bool set_selected) {
  ...
  if (ImGui::BeginTabItem("Motion Frames", nullptr, flags)) {
    ...
    if (server_cameras_.empty() ||
        (current_time - last_server_camera_fetch_time_ > fetch_interval)) {
    ...
    if (server_cameras_.empty()) {
      if (server_camera_fetch_in_progress_) {
        ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f),
                           "Loading cameras from server...");
      } else {
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                           "No cameras available on server");
    ...
    if (selected_camera_index_ < 0 ||
        selected_camera_index_ >= static_cast<int>(server_cameras_.size())) {
      ...
      if (ImGui::MenuItem("Remove Camera")) {
        if (async_worker_) {
          std::string camera_name =
              server_cameras_[selected_camera_index_].name;
          std::string endpoint = std::string(server_endpoint_.data());
    ...
    if (ImGui::BeginCombo(
            "##camera_select",
            server_cameras_[selected_camera_index_].name.c_str())) {
    ...
    // Motion toggle
    ImGui::Text("Motion Detection Status: %s",
    ...
    ImGui::SliderFloat("Frame Fetch Interval (s)",
                       &motion_frame_fetch_interval_, 0.1f, 5.0f, "%.1f");
    ImGui::PopItemWidth();
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("How often to fetch motion frames from the server "
                        "(lower = more frequent)");
    ...
    if (segment_enabled) {
      if (ImGui::Button("Disable Record on Motion")) {
        if (async_worker_) {
          ...
          std::string endpoint = std::string(server_endpoint_.data());
          async_worker_->enqueueTask([camera_name, endpoint]() {
            client_network::toggle_segment_recording(endpoint, camera_name,
                                                     false);
          });
        }
      }
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip(
          "Enable continuous segment recording that saves only when motion is "
    ...
    if (motion_enabled && !toggled) {
      ...
      // Process fetched JPEG data on main thread FIRST (decode + texture upload
      // must be on main thread) This must happen before checking if we should
      // start a new fetch, so timestamp is updated properly
      if (has_pending_motion_frame_) {
        ...
        std::vector<unsigned char> jpeg_data_copy;
        {
          ...
          MOTION_FRAME_LOG("Main thread: JPEG data size in buffer: "
                           << jpeg_data_copy.size() << " bytes");
        }

        // Decode JPEG and create/update SDL texture directly from buffered data
        MOTION_FRAME_LOG(
            "Main thread: Decoding JPEG from buffer (no re-fetch)");
        decode_motion_frame_from_buffer_(jpeg_data_copy, motion_frame_texture_,
                                         motion_frame_width_,
                                         motion_frame_height_);

        // Update timestamp AFTER decode completes (on main thread with valid
        // ImGui::GetTime())
        last_motion_frame_fetch_ = current_time;
        MOTION_FRAME_LOG("Main thread: Decode complete, timestamp updated");
      }

      // NOW check if we should start a new fetch (after processing any pending
      // frame)
      if (current_time - last_motion_frame_fetch_ >
          motion_frame_fetch_interval_) {
        if (!motion_frame_fetch_in_progress_ && motion_frame_worker_) {
          MOTION_FRAME_LOG("Starting async fetch for camera: "
                           << selected_camera.name << " (interval: "
                           << motion_frame_fetch_interval_ << "s)");
          motion_frame_fetch_in_progress_ = true;
          // Don't update last_motion_frame_fetch_ here - will update after
          // decode completes

          std::string camera_name = selected_camera.name;
          std::string endpoint(server_endpoint_.data());

          // Use dedicated motion frame worker (not shared with other async
          // tasks) This ensures motion frame fetching is never delayed by
          // health checks, camera lists, etc.
          motion_frame_worker_->enqueueTask([this, camera_name, endpoint]() {
            MOTION_FRAME_LOG(
                "Background thread: Fetching JPEG from network for "
                << camera_name);
            std::vector<unsigned char> jpeg_buffer;

            // Network I/O happens in background - doesn't block UI
            bool success = client_network::fetch_motion_frame_jpeg(
                endpoint, camera_name, jpeg_buffer);

            if (success && !jpeg_buffer.empty()) {
              MOTION_FRAME_LOG("Background thread: JPEG fetch SUCCESS, size="
                               << jpeg_buffer.size() << " bytes");
              std::lock_guard<std::mutex> lock(motion_frame_mutex_);
          ...
          last_motion_frame_fetch_ = current_time;
          fetch_motion_frame_callback_(
              selected_camera.name, motion_frame_texture_, motion_frame_width_,
              motion_frame_height_);
        }
      }

      if (motion_frame_texture_ && motion_frame_width_ > 0 &&
          motion_frame_height_ > 0) {
        // Fetch regions for selected camera periodically (async to avoid
        // blocking UI)
        if (get_motion_regions_callback_ && async_worker_ &&
            (current_time - last_region_fetch_time_ > 2.0f)) {
          std::string camera_name = selected_camera.name;
          async_worker_->enqueueTask([this, camera_name]() {
        ...
        float aspect = static_cast<float>(motion_frame_width_) /
                       static_cast<float>(motion_frame_height_);
        float display_width =
            std::min(max_width, static_cast<float>(motion_frame_width_));
        float display_height = display_width / aspect;
        float scale_x = display_width / static_cast<float>(motion_frame_width_);
        float scale_y =
            display_height / static_cast<float>(motion_frame_height_);

        ImVec2 image_pos = ImGui::GetCursorScreenPos();
        ImGui::Image(motion_frame_texture_,
                     ImVec2(display_width, display_height));

        // Define image bounds for mouse interaction
        ImVec2 image_min = image_pos;
        ImVec2 image_max(image_pos.x + display_width,
                         image_pos.y + display_height);
        ImVec2 mouse_pos = ImGui::GetMousePos();
        bool mouse_in_image =
            mouse_pos.x >= image_min.x && mouse_pos.x <= image_max.x &&
            mouse_pos.y >= image_min.y && mouse_pos.y <= image_max.y;

        // Handle mouse interaction for drawing regions
        if (mouse_in_image) {
          ImVec2 relative_pos(mouse_pos.x - image_pos.x,
                              mouse_pos.y - image_pos.y);

          if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
              !drawing_motion_region_) {
        ...
        ImDrawList *draw_list = ImGui::GetWindowDrawList();
        {
          ...
          for (const auto &region : motion_regions_) {
            ...
            if (std::abs(region.angle) > 0.001f) {
              ...
              float hw = rw / 2.0f;
              float hh = rh / 2.0f;
              corners[0] = ImVec2(cx + (-hw * cos_a - (-hh) * sin_a),
                                  cy + (-hw * sin_a + (-hh) * cos_a));
              corners[1] = ImVec2(cx + (hw * cos_a - (-hh) * sin_a),
                                  cy + (hw * sin_a + (-hh) * cos_a));
              corners[2] = ImVec2(cx + (hw * cos_a - hh * sin_a),
                                  cy + (hw * sin_a + hh * cos_a));
              corners[3] = ImVec2(cx + (-hw * cos_a - hh * sin_a),
                                  cy + (-hw * sin_a + hh * cos_a));

              for (int i = 0; i < 4; i++)
                draw_list->AddLine(corners[i], corners[(i + 1) % 4],
                                   IM_COL32(0, 255, 0, 255), 2.0f);
            } else {
              draw_list->AddRect(ImVec2(rx, ry), ImVec2(rx + rw, ry + rh),
                                 IM_COL32(0, 255, 0, 255), 0.0f, 0, 2.0f);
            }
            draw_list->AddText(ImVec2(rx + 5, ry + 5),
        ...
        if (drawing_motion_region_) {
          float x1 =
              image_pos.x + std::min(region_draw_start_x_, region_draw_end_x_);
          float y1 =
              image_pos.y + std::min(region_draw_start_y_, region_draw_end_y_);
          float x2 =
              image_pos.x + std::max(region_draw_start_x_, region_draw_end_x_);
          float y2 =
              image_pos.y + std::max(region_draw_start_y_, region_draw_end_y_);
          draw_list->AddRect(ImVec2(x1, y1), ImVec2(x2, y2),
                             IM_COL32(255, 255, 0, 255), 0.0f, 0, 2.0f);
        }

        // Draw pending region in blue with rotation preview
        bool has_drawn_region =
            !drawing_motion_region_ &&
        ...
        ImGui::TextDisabled(
            "Click and drag on the image to draw a motion region.");
        ...
        if (has_drawn_region) {
          ...
          ImGui::SliderFloat("Region Angle", &pending_region_angle_, -180.0f,
                             180.0f, "%.1f°");
          ImGui::PopItemWidth();
          ImGui::SameLine();
          ImGui::TextDisabled("(?)");
        ...
        if (ImGui::Button("Save Motion Region")) {
          if (add_motion_region_callback_ && async_worker_ &&
              has_drawn_region) {
            // Convert display coordinates to actual image coordinates
            int x = static_cast<int>(
                std::min(region_draw_start_x_, region_draw_end_x_) / scale_x);
            int y = static_cast<int>(
                std::min(region_draw_start_y_, region_draw_end_y_) / scale_y);
            int w = static_cast<int>(
                std::abs(region_draw_end_x_ - region_draw_start_x_) / scale_x);
            int h = static_cast<int>(
                std::abs(region_draw_end_y_ - region_draw_start_y_) / scale_y);
            ...
            async_worker_->enqueueTask([this, camera_name, x, y, w, h,
                                        angle]() {
              int region_id =
                  add_motion_region_callback_(camera_name, x, y, w, h, angle);
        ...
        if (region_count > 0) {
          ...
          if (selected_region_index_ < 0 ||
              selected_region_index_ >=
                  static_cast<int>(motion_regions_.size())) {
        ...
        static float temp_motion_frame_scale =
            selected_camera.motion_frame_scale;
        ...
        static float temp_motion_arrow_scale =
            selected_camera.motion_arrow_scale;
        static int temp_motion_arrow_thickness =
            selected_camera.motion_arrow_thickness;
        ...
        if (ImGui::SliderFloat("Motion Frame Scale", &temp_motion_frame_scale,
                               0.1f, 2.0f, "%.2f"))
        ...
        if (has_motion_regions &&
            ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
          ImGui::SetTooltip(
              "Cannot change scale while motion regions are defined.\nClear "
              "all regions first to adjust this setting.");
        }
        ...
        if (ImGui::IsItemHovered()) {
          if (has_motion_regions) {
            ...
            ImGui::SetTooltip("Scale factor for motion frame size. Lower = "
                              "faster processing.");
          }
        }
        ...
        if (ImGui::SliderFloat("Noise Threshold", &temp_noise_threshold, 0.0f,
                               10.0f, "%.2f"))
          properties_changed = true;
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "Minimum pixel movement to consider (filters out noise).");

        if (ImGui::SliderFloat("Motion Threshold", &temp_motion_threshold, 0.0f,
                               50.0f, "%.2f"))
          properties_changed = true;
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        ...
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "Number of consecutive frames with motion to trigger.");
        ...
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "How quickly motion counter decreases when no motion.");

        if (ImGui::SliderFloat("Arrow Scale", &temp_motion_arrow_scale, 1.0f,
                               10.0f, "%.1f"))
          properties_changed = true;
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        ...
        if (ImGui::SliderInt("Arrow Thickness", &temp_motion_arrow_thickness, 1,
                             5))
          properties_changed = true;
        ImGui::SameLine();
        ImGui::TextDisabled("(?)");
        ...
        if (ImGui::Button("Apply Changes") ||
            (properties_changed && ImGui::IsKeyPressed(ImGuiKey_Enter))) {
          if (async_worker_) {
            std::string camera_name = selected_camera.name;
            std::string endpoint(server_endpoint_.data());
            ...
            async_worker_->enqueueTask(
                [endpoint, camera_name, motion_frame_scale, noise_threshold,
                 motion_threshold, motion_min_hits, motion_decay,
                 motion_arrow_scale, motion_arrow_thickness]() {
                  client_network::update_camera_properties(
                      endpoint, camera_name, motion_frame_scale,
                      noise_threshold, motion_threshold, motion_min_hits,
        ...
        ImGui::TextUnformatted(
            "Motion frame will appear when motion is detected.");
      }
  ...
  if (ImGui::BeginTabItem("Info", nullptr, flags)) {
    ...
    if (thread_info_callback_) {
      ...
      if (threads.empty()) {
        ...
        if (ImGui::BeginTable("ThreadTable", 3,
          ...
          ImGui::TableSetupColumn("Thread Name",
          ...
          for (const auto &thread : threads) {
            ...
            ImVec4 status_color = thread.is_active
                                      ? ImVec4(0.2f, 0.8f, 0.2f, 1.0f)
                                      : ImVec4(0.7f, 0.7f, 0.7f, 1.0f);
            ImGui::TextColored(status_color, "%s",
                               thread.is_active ? "Active" : "Stopped");
    ...
    if (!endpoint.empty()) {
      ...
      if (!cached_server_threads_.empty()) {
        ...
        if (ImGui::BeginTable("ServerThreadTable", 3,
                              ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg |
                                  ImGuiTableFlags_SizingStretchProp)) {
          ImGui::TableSetupColumn("Thread Name",
                                  ImGuiTableColumnFlags_WidthFixed, 200.0f);
