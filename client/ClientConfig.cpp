...
ClientConfig load_client_config(const std::filesystem::path &config_path,
    ...
    try {
        ...
        throw std::runtime_error(std::string("Failed to parse config file: ") + err.what());
    }
    ...
    if (server_it != json_doc.end() && server_it->is_object()) {
        ...
        if (auto ip_it = server_obj.find("serverIP"); ip_it != server_obj.end() && ip_it->is_string()) {
            config.server_ip = ip_it->get<std::string>();
        }
        if (auto cameras_it = server_obj.find("cameras"); cameras_it != server_obj.end()) {
            ...
            if (cameras_node.is_array()) {
                for (const auto &camera_entry : cameras_node) {
                    ...
                    if (camera_entry.is_object()) {
                        if (auto nested = camera_entry.find("camera"); nested != camera_entry.end() && nested->is_object()) {
                            camera_obj = &(*nested);
                        } else {
                    ...
                    CameraConfig camera;
                    if (auto name_it = camera_obj->find("name"); name_it != camera_obj->end() && name_it->is_string()) {
                        camera.name = name_it->get<std::string>();
                    }
                    if (auto ip_it = camera_obj->find("ip"); ip_it != camera_obj->end() && ip_it->is_string()) {
                        camera.ip = ip_it->get<std::string>();
                    }
                    if (auto via_it = camera_obj->find("viaServer"); via_it != camera_obj->end() && via_it->is_boolean()) {
                        camera.via_server = via_it->get<bool>();
                    }
                    if (auto orig_it = camera_obj->find("originalUri"); orig_it != camera_obj->end() && orig_it->is_string()) {
                        camera.original_uri = orig_it->get<std::string>();
                    }

                    const nlohmann::json *options_obj = nullptr;
                    if (auto options_it = camera_obj->find("options"); options_it != camera_obj->end() && options_it->is_object()) {
                    ...
                    auto assign_bool = [&](const char *key, bool &target) {
                        if (auto it = option_source.find(key); it != option_source.end() && it->is_boolean()) {
                            target = it->get<bool>();
                        }
                    };
                    auto assign_int = [&](const char *key, int &target) {
                        if (auto it = option_source.find(key); it != option_source.end() && it->is_number_integer()) {
                            target = it->get<int>();
                        }
                    };
                    auto assign_float = [&](const char *key, float &target) {
                        if (auto it = option_source.find(key); it != option_source.end() && it->is_number()) {
                            target = static_cast<float>(it->get<double>());
                        }
                    };
                    auto assign_string = [&](const char *key, std::string &target) {
                        if (auto it = option_source.find(key); it != option_source.end() && it->is_string()) {
    ...
    if (configuration_it != json_doc.end() && configuration_it->is_object()) {
        ...
        if (auto general_it = configuration_obj.find("general"); general_it != configuration_obj.end() && general_it->is_object()) {
            const auto &general_obj = *general_it;
            if (auto width_it = general_obj.find("windowWidth"); width_it != general_obj.end() && width_it->is_number()) {
                config.window_settings.width = static_cast<float>(width_it->get<double>());
            }
            if (auto height_it = general_obj.find("windowHeight"); height_it != general_obj.end() && height_it->is_number()) {
                config.window_settings.height = static_cast<float>(height_it->get<double>());
            }
            if (auto metrics_it = general_obj.find("showImGuiMetrics"); metrics_it != general_obj.end() && metrics_it->is_boolean()) {
                config.window_settings.show_imgui_metrics = metrics_it->get<bool>();
            }
            if (auto endpoint_it = general_obj.find("serverEndpoint"); endpoint_it != general_obj.end() && endpoint_it->is_string()) {
...
void save_client_config(const nlohmann::json &json_doc, const std::filesystem::path &config_path) {
    ...
    if (!parent.empty()) {
        ...
        if (ec) {
            throw std::runtime_error("Unable to create config directory: " + parent.string() + " (" + ec.message() + ")");
        }
    }

    std::ofstream output(config_path);
    if (!output) {
        throw std::runtime_error("Unable to write config file: " + config_path.string());
    }
