#pragma once
#include <Qt> // for Qt::Key
// Some client default setttings, TODO: firsthand read from json instead
namespace ClientSettings {

// Log to file in addition to stdout?
inline constexpr bool LOG_TO_FILE = true; // ← toggle this
inline const char *LOG_FILE_PATH = "richnvr.log";
inline constexpr bool LOG_OVERWRITE_ON_START =
    true; // ← overwrite logfile by default
...
inline const QStringList VIDEOSINK_FALLBACKS = {
    "qt6glsink",    // Qt-specific, if plugin is present
    "glimagesink",  // cross-platform OpenGL sink
    "autovideosink" // “whatever works” generic sink
};
...
constexpr int MOTION_TIME = 7; // 7s

// When drawing motion-vectors
inline constexpr float MOTION_ARROW_SCALE = 2.5f;
inline constexpr int MOTION_ARROW_THICKNESS = 1;
inline const QString VIDEO_OUTPUT_FORMAT = "mp4"; // mp4, mkv, webm, gif, etc.#

inline constexpr int motionFramePollIntervalMs = 20; // default 1s
