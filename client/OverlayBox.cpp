...
void OverlayBox::contextMenuEvent(QContextMenuEvent *e) {
  QMenu menu(this);
  // menu.addAction("Start Recording", this, [this] { emit startRecording(); });
  // menu.addAction("Take Snapshot", this, [this] { emit snapshot(); });
  // menu.addSeparator();
  // menu.addAction("Toggle Fullscreen", this,
...
void OverlayBox::setupOverlayButtons() {
  ...
  connect(motionBtn, &QPushButton::clicked, this, [this, motionBtn] {
    motionOn_ = !motionOn_;
    // motionBtn->setText(motionOn_ ? "Motion-On" : "Motion");
...
void OverlayBox::sendMotionUpdate(bool enable) {
  ...
  // Endpoint URL (can move to Settings)
  QUrl url(
  ...
  QObject::connect(reply, &QNetworkReply::finished, reply, [this, reply] {
    if (reply->error() == QNetworkReply::NoError) {
      ...
      // std::cout << "[Overlay] Motion update failed:" << reply->errorString();
