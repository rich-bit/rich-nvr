...
static std::unordered_map<std::string, std::shared_ptr<httplib::Client>>
    client_cache;
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
static std::unordered_map<std::string, std::shared_ptr<httplib::SSLClient>>
    ssl_client_cache;
#endif
...
EndpointParts parse_endpoint(const std::string &endpoint) {
  ...
  std::string host_segment =
      slash_pos == std::string::npos ? working : working.substr(0, slash_pos);
  ...
  if (!host_segment.empty()) {
    ...
    if (host_segment.front() == '[') {
      ...
      if (closing != std::string::npos) {
        ...
        if (closing + 1 < host_segment.size() &&
            host_segment[closing + 1] == ':') {
...
get_or_create_ssl_client(const std::string &host, int port) {
  // Normalize localhost to 127.0.0.1 on Windows before caching
  std::string normalized_host = normalize_endpoint_for_cache(host);
  std::string cache_key = normalized_host + ":" + std::to_string(port);
...
std::string extract_host_from_endpoint(const std::string &endpoint) {
  ...
  if (auto scheme_pos = host_part.find("://");
...
AddCameraResult send_add_camera_request(const AddCameraRequest &request,
  ...
  if (request.motion_frame_width > 0) {
    params.emplace("motion_frame_w",
                   std::to_string(request.motion_frame_width));
  }
  if (request.motion_frame_height > 0) {
    params.emplace("motion_frame_h",
                   std::to_string(request.motion_frame_height));
  }
  if (request.motion_frame_scale > 0.0f) {
    params.emplace("motion_frame_scale",
                   format_float(request.motion_frame_scale));
  }
  ...
  if (request.motion_arrow_scale > 0.0f) {
    params.emplace("motion_arrow_scale",
                   format_float(request.motion_arrow_scale));
  }
  if (request.motion_arrow_thickness > 0) {
    params.emplace("motion_arrow_thickness",
                   std::to_string(request.motion_arrow_thickness));
  }
  ...
  auto perform_request = [&](auto client_ptr) -> AddCameraResult {
    ...
    if (res->status < 200 || res->status >= 300) {
      inner_result.message =
          response_body.empty()
              ? ("Server returned status " + std::to_string(res->status))
              : response_body;
      return inner_result;
    }
    ...
    inner_result.message =
        response_body.empty() ? "Camera added via NVR Server." : response_body;
    return inner_result;
  };
...
get_motion_regions(const std::string &endpoint,
                   const std::string &camera_name) {
  ...
  auto perform_request =
      [&](auto client_ptr) -> std::vector<ConfigurationPanel::MotionRegion> {
    ...
    try {
      ...
      if (j.contains("regions") && j["regions"].is_array()) {
        if (DEBUG_MOTION_FRAME) {
          std::cout << "[get_motion_regions] Found " << j["regions"].size()
                    << " regions" << std::endl;
        }
        for (const auto &region_json : j["regions"]) {
          ...
          if (DEBUG_MOTION_FRAME) {
            std::cout << "  Region " << region.id << ": " << region.name
                      << " at (" << region.x << "," << region.y << ") size "
                      << region.w << "x" << region.h << std::endl;
          }
          result.push_back(region);
        }
      } else {
        if (DEBUG_MOTION_FRAME) {
          std::cout << "[get_motion_regions] No 'regions' array in response"
                    << std::endl;
        }
      }
    } catch (const std::exception &e) {
      if (DEBUG_MOTION_FRAME) {
        std::cout << "[get_motion_regions] JSON parse error: " << e.what()
                  << std::endl;
      }
      return result;
    }
...
bool update_camera_properties(const std::string &endpoint,
  ...
  params.emplace("motion_arrow_thickness",
                 std::to_string(motion_arrow_thickness));
...
bool remove_camera(const std::string &endpoint,
                   const std::string &camera_name) {
  EndpointParts parts = parse_endpoint(endpoint);
  if (parts.host.empty()) {
    return false;
...
get_cameras_from_server(const std::string &endpoint) {
  ...
  auto perform_request =
      [&](auto client_ptr) -> std::vector<ConfigurationPanel::CameraInfo> {
    ...
    try {
      ...
      if (j.is_array()) {
        for (const auto &cam_json : j) {
          ...
          info.motion_arrow_thickness =
              cam_json.value("motion_arrow_thickness", 1);
          result.push_back(info);
        }
      }
    } catch (const std::exception &e) {
      if (DEBUG_MOTION_FRAME) {
        std::cout << "[get_cameras_from_server] JSON parse error: " << e.what()
                  << std::endl;
      }
      return result;
    }
