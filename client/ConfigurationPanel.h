...
public:
  enum class Tab { General, Audio, AddCamera, MotionFrame, Info };
  ...
  using PersistCallback =
      std::function<void(const ConfigurationWindowSettings &)>;
  using AddCameraCallback =
      std::function<AddCameraResult(const AddCameraRequest &)>;
  using ProbeStreamCallback =
      std::function<ProbeStreamResult(const std::string &)>;
  ...
  using GetRTSPConfigCallback =
      std::function<client_config::CameraConfig &(int)>;
  ...
  ConfigurationPanel(
      ...
      std::function<bool(const std::string &, int)>
          remove_motion_region_callback,
  ...
  const client_config::CameraConfig &getTempRTSPConfig() const {
    return *rtsp_config_temp_;
  }
  ...
  void setAsyncWorker(class AsyncNetworkWorker *worker) {
    async_worker_ = worker;
  }

  // Public accessor for motion frame worker (for thread info display)
  class AsyncNetworkWorker *getMotionFrameWorker() const {
    return motion_frame_worker_.get();
  }
...
private:
  ...
  void decode_motion_frame_from_buffer_(
  ...
  std::unique_ptr<class AsyncNetworkWorker>
      motion_frame_worker_; // Dedicated worker for motion frames
  ...
  std::unique_ptr<client_config::CameraConfig>
      rtsp_config_temp_; // Temporary config for Add Camera mode
